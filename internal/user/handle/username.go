// Package handle holds the pure, dependency-free username + handle logic:
// canonical-username validation and best-effort handle generation. It imports
// nothing from internal/user (or any other internal package), so leaf consumers
// such as internal/db (the username backfill migration) can reuse the exact
// same logic without creating an import cycle. internal/user re-exports these
// symbols so existing call sites keep using user.ValidateUsername /
// user.GenerateHandle unchanged.
package handle
...
// Username write-path errors. Invalid covers charset/length/shape (input the
// regex rejects); Reserved is a structurally-valid but denylisted name. (Taken
// — a unique-index collision — is surfaced by the repository layer, not here,
// so it stays in internal/user.)
var (
