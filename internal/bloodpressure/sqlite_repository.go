...
func (r *SQLiteRepository) Create(ctx context.Context, e *Entry) error {
	...
	// Bind pulse as NULL when absent so the "not measured" state round-trips
	// as SQL NULL rather than 0.
	var pulseArg any
	if e.Pulse != nil {
		pulseArg = *e.Pulse
	}
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO user_blood_pressure (
			id, user_id, systolic, diastolic, pulse, measured_at, created_at
		) VALUES (?, ?, ?, ?, ?, ?, ?)
	`, e.ID, e.UserID, e.Systolic, e.Diastolic, pulseArg, e.MeasuredAt, e.CreatedAt)
	return err
}

func (r *SQLiteRepository) Get(ctx context.Context, userID, entryID string) (*Entry, error) {
	row := r.db.QueryRowContext(ctx, `
		SELECT id, user_id, systolic, diastolic, pulse, measured_at, created_at, deleted_at
...
func (r *SQLiteRepository) UpdateEntry(ctx context.Context, e *Entry) error {
	...
	res, err := r.db.ExecContext(ctx, `
		UPDATE user_blood_pressure
		SET systolic = ?, diastolic = ?, pulse = ?, measured_at = ?
		WHERE id = ? AND user_id = ? AND deleted_at IS NULL
	`, e.Systolic, e.Diastolic, pulseArg, e.MeasuredAt, e.ID, e.UserID)
	if err != nil {
		return err
	}
...
func scanEntry(s scanner) (*Entry, error) {
	...
	if err := s.Scan(
		&e.ID, &e.UserID, &e.Systolic, &e.Diastolic, &pulse, &e.MeasuredAt, &e.CreatedAt, &deletedAt,
	); err != nil {
		return nil, err
	}
	if pulse.Valid {
