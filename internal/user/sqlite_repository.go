...
func (r *SQLiteRepository) Create(ctx context.Context, u *User) error {
	...
	_, err := r.db.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?, ?, ?)
	`, u.ID, u.Email, u.DisplayName, u.WeightUnit, u.DistanceUnit, u.CreatedAt, u.UpdatedAt)
...
func (r *SQLiteRepository) GetByID(ctx context.Context, id string) (*User, error) {
	...
	err := r.db.QueryRowContext(ctx, `
		SELECT id, email, display_name, weight_unit, distance_unit, created_at, updated_at, deleted_at
		FROM users
		WHERE id = ? AND deleted_at IS NULL
	`, id).Scan(&u.ID, &u.Email, &u.DisplayName, &u.WeightUnit, &u.DistanceUnit, &u.CreatedAt, &u.UpdatedAt, &u.DeletedAt)
...
func (r *SQLiteRepository) GetByEmail(ctx context.Context, email string) (*User, error) {
	...
	`, email).Scan(&u.ID, &u.Email, &u.DisplayName, &u.WeightUnit, &u.DistanceUnit, &u.CreatedAt, &u.UpdatedAt, &u.DeletedAt)
...
func (r *SQLiteRepository) Update(ctx context.Context, u *User) error {
	...
	result, err := r.db.ExecContext(ctx, `
		UPDATE users
		SET display_name = ?, weight_unit = ?, distance_unit = ?, updated_at = ?
		WHERE id = ? AND deleted_at IS NULL
	`, u.DisplayName, u.WeightUnit, u.DistanceUnit, u.UpdatedAt, u.ID)
