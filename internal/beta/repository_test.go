...
// repoFactory builds a fresh Repository for a backend, plus a hook to set an
// injectable clock so List-ordering tests can stagger added_at deterministically.
type repoFactory struct {
	name   string
	build  func(t *testing.T) Repository
	setNow func(r Repository, now func() time.Time)
}

func backends() []repoFactory {
	return []repoFactory{
		{
			name:  "sqlite",
			build: func(t *testing.T) Repository { return newSQLiteBetaRepo(t) },
			setNow: func(r Repository, now func() time.Time) {
				r.(*SQLiteRepository).now = now
			},
		},
		{
			name:  "memory",
			build: func(t *testing.T) Repository { return NewMemoryRepository() },
			setNow: func(r Repository, now func() time.Time) {
				r.(*MemoryRepository).now = now
			},
		},
	}
}
