// Package authctx holds the request-context plumbing for the authenticated
// user ID. It lives in its own leaf package (no other internal imports) so
// packages that auth itself depends on — notably user — can read the user
// ID from context without forming an import cycle (auth imports user, so
// user cannot import auth).
package authctx
